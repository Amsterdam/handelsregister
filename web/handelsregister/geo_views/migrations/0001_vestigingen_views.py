...
"""
    ...
    WHERE sbi_detailgroep in ('bouw overig', 'bouwinstallatie', 'afwerking van gebouwen',
    ...
    WHERE sbi_detailgroep in ('installatie (geen bouw)', 'reparatie (geen bouw)', 'productie')"""
        ...
        migrate.ManageView(
            ...
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('arbeidsbemiddeling, uitzendbureaus, uitleenbureaus',
      'overige zakelijke dienstverlening', 'reclame en marktonderzoek', 'interieurarchitecten',
      'managementadvies, economisch advies', 'technisch ontwerp, advies, keuring/research', 'design',
      'public relationsbureaus', 'advocaten rechtskundige diensten, notarissen', 'architecten',
      'accountancy, administratie')"""
        ...
        migrate.ManageView(
            ...
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('vervoer', 'detailhandel (verkoop aan consumenten, niet zelf vervaardigd)',
        'opslag', 'dienstverlening vervoer', 'groothandel (verkoop aan andere ondernemingen, niet zelf vervaardigd)')"""
        ),
        migrate.ManageView(
            view_name="geo_hr_vestiging_locaties_landbouw",
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('teelt eenjarige gewassen', 'gemengd bedrijf', 'fokken, houden dieren',
      'teelt sierplanten', 'dienstverlening voor de land/tuinbouw', 'teelt meerjarige gewassen')"""
        ),
        migrate.ManageView(
            view_name="geo_hr_vestiging_locaties_persoonlijke_dienstverlening",
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('sauna, solaria', 'schoonheidsverzorging', 'uitvaart, crematoria',
      'overige dienstverlening', 'kappers')"""
        ...
        migrate.ManageView(
            ...
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('activiteiten op het gebied van ict',
      'activiteiten  op gebied van film, tv, radio, audio', 'telecommunicatie, uitgeverijen')"""
        ),
        migrate.ManageView(
            view_name="geo_hr_vestiging_locaties_cultuur_sport_recreatie",
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('sport', 'musea, bibliotheken, kunstuitleen', 'kunst, recreatie')"""
        ),
        migrate.ManageView(
            view_name="geo_hr_vestiging_locaties_financiele_dienstverlening_verhuur",
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('verhuur van- en beheer/handel in onroerend goed',
      'verhuur van roerende goederen', 'holdings, financiële dienstverlening en verzekeringen')"""
        ),
        migrate.ManageView(
            view_name="geo_hr_vestiging_locaties_overige",
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('idieële organisaties', 'belangenorganisaties', 'overige', 'hobbyclubs')"""
        ),
        migrate.ManageView(
            view_name="geo_hr_vestiging_locaties_horeca",
            sql="""
    SELECT * FROM geo_hr_vestiging_locaties
    WHERE sbi_detailgroep in ('hotel-restaurant', 'overige horeca', 'kantine, catering',
      'cafetaria, snackbar, ijssalon', 'café', 'hotel, pension', 'restaurant, café-restaurant')"""
        ),
